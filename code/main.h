...
void rectangle(struct Png *image, int rgb_old[], int rgb_new[])
{
    ...
    {
        fprintf(stderr, "Image's type color must be RGB or RGBA\n");
        exit(1);
    }

    int Area[2] = {0};
    int height = 0, hf = 0, heightf = 0, lengthk = 0, lengthj = 0;
    ...
    for (int i = 0; i < image->height; i++)
    {
        ...
        for (int j = 0; j < image->width; j++)
        {
            ...
            if (rgb_old[0] == ptr[0] && rgb_old[1] == ptr[1] && rgb_old[2] == ptr[2])
            {
                ...
                int q = 0;
                if ((j != lengthj && k != lengthk) || (j == lengthj && k != lengthk))
                {
...
void collage(struct Png *image, int m, int n)
{
    ...
    {
        fprintf(stderr, "error, the color type of image must be RGB or RGBA\n");
        exit(0);
    }
    ...
    png_bytep *rowPointers = (png_bytep *)calloc(h, sizeof(png_bytep));
    for (int y = 0; y < h; y++)
    {
        rowPointers[y] = (png_byte *)calloc(w, sizeof(png_byte));
        rowPointers[y] = image->row_pointers[y];
    }
    ...
    for (int j = 0; j < m; j++)
        for (int i = 0; i < n; i++)
            for (int y = 0; y < h; y++)
                for (int x = 0; x < w * typecolor; x++)
                    image->row_pointers[y + (j * h)][x + (i * w * typecolor)] = rowPointers[y][x];
